...
### Tests
- `apps/engine/tests/test_config.py`: +3 tests para `config.get_settings()` que estaba sin cubrir (línea 24 — `return Settings()`). El resto de la suite stubea `main.get_settings` con lambdas y nunca invoca el real, por lo que el `lru_cache` quedaba sin tocar. Tests: instancia válida, idempotencia por `lru_cache` (mismo objeto en llamadas sucesivas), defaults documentados (`engine_api_key='changeme-internal-only'`, `log_level='info'`, `balance_tolerancia_pct=0.005`). Cobertura `config.py` 92% → 100%. Suite 108 verdes (era 105), ruff `check .` limpio. — 2026-05-20
- `apps/engine/tests/test_financial.py`: +4 tests cubriendo `tornado_chart` que estaba sin tests (líneas 133-154 de `financial.py`). Casos: ordenamiento por magnitud absoluta del delta TIR, `base_tir=None` deja ambos impactos en None, `kpis_baja.tir=None` deja sólo `impacto_tir_baja=None`, lista vacía retorna `[]`. Cobertura `financial.py` 93% → 98%. Suite 104 verdes (era 100), ruff `check .` limpio. — 2026-05-20
- `apps/engine/tests/test_mass_balance.py`: +6 tests cerrando branches sin cubrir en `mass_balance.py` — constructor con humedad+ms fuera de tolerancia (línea 48), `perdidas_pct >= 1` (línea 145), extracciones que exceden MS pura en modo B (línea 119), `humedad_final_pct=0.99` que fuerza `agua_evaporada < 0` (línea 157), sankey con `licopeno`/`pectina` extraídos (líneas 218 y 222), property `materia_seca_neta_pct` con `input_ton=0` (línea 79). Cobertura `mass_balance.py` 91% → 99% (sólo queda línea 164, guard defensivo inalcanzable por construcción). Suite 97 verdes (era 91), ruff `check .` limpio. — 2026-05-19
